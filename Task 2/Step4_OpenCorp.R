...
# Warning:
# Open Corporates provides free API access for up to (about)
# 50 MNEs. After that it provides an error that no more data
# is allowed to be downloaded.
#
# Furthermore, it marks the IP address and once the user exceeds
# the limit, then no data can be downloaded at all.
#
...
query <- "https://api.opencorporates.com/companies/"
queryg <- "?format=json"
...
for(i in 1:NROW(x))
{
  ...
  # use the JSON and make the call
  json_data <- fromJSON(file=queryf)
  json_data <- json_data$results$company
  
  # unlist the items
  json_data2 <- flatten(json_data)
  
  # unlist again and save them, also put the MNE name
...
write.csv(all_stacked, "OpenCorp_data.csv")
