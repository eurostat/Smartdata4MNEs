...
#########################################################
# Load data
fname <- paste(filesdir1, "EDGAR_data.csv", sep="")
...
write.csv(fdd, paste(filesdir2, "RetrievalDate7_EDGAR.csv", sep=""))

# Obtain unique fields:
unqf <- unique(as.character(x[,3]))

# save the fields output
write.csv(unqf, paste(filesdir3, "Fields7_EDGAR.csv", sep=""))

#########################################################
# If first pass matching has been finalised
# then load newly created matrix which has the varlist here
varlist <- read.csv(paste(filesdir4, "Varlist7_EDGAR.csv", sep=""), header=TRUE)
  ...
  if(NROW(xtemp)==1){
    ...
    for(i in 1:NROW(vlist))
    {
      ...
      if(NROW(ipos)==0){
        ...
        xfield <- xtemp[ipos,6]
        xdetailtype <- xtemp[ipos,4]
        xdetailvalu <- xtemp[ipos,5]
      }
...
write.csv(fdb, paste(filesdir5, "Output7_EDGAR.csv", sep=""))
