...
#########################################################
# Load data
fname <- paste(filesdir1, "All_companies_PERMID.csv", sep="")
...
write.csv(fdd, paste(filesdir2, "RetrievalDate5_PERMID.csv", sep=""))

# Obtain unique fields:
unqf <- unique(as.character(x[,3]))

# save the fields output
write.csv(unqf, paste(filesdir3, "Fields5_PERMID.csv", sep=""))

#########################################################
# If first pass matching has been finalised
# then load newly created matrix which has the varlist here
varlist <- read.csv(paste(filesdir4, "Varlist5_PermID.csv", sep=""), header=TRUE)
...
for(j in 1:NROW(N))
{
  ...
  if(NROW(xtemp)==1){
    xout <- cbind(rep(jcompany, NROW(vlist)), vlist[,2],
                  1:NROW(vlist), rep(NA, NROW(vlist)), rep(NA, NROW(vlist)))
    fdb <- rbind(fdb, xout)
  }else{
    ...
    {
      if((xtemp[i,4]=="Position")|
         (xtemp[i,4]=="Title")|
         (xtemp[i,4]=="Start Date")){
        ifsel <- c(ifsel, i)
      }
    }
    xfield <- cbind(xtemp[ifsel,4], ifsel, xtemp[ifsel,3], xtemp[ifsel,5])
    xout <- cbind(rep(jcompany, NROW(xfield)), xfield)
    fdb <- rbind(fdb, xout)
  }
}
rownames(fdb) <- NULL
colnames(fdb) <- c("ESTAT_MNE", "Var", "VarID","Value", "VarDetailValue")

write.csv(fdb, paste(filesdir5, "Output5_PERMID.csv", sep=""))
