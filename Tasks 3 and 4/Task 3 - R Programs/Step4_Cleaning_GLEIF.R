...
#########################################################
# Load data
fname <- paste(filesdir1, "GLEIF-cDATA.csv", sep="")
...
write.csv(fdd, paste(filesdir2, "RetrievalDate4_GLEIF.csv", sep=""))

# Obtain unique fields:
unqf <- unique(as.character(x[,3]))

# save the fields output
write.csv(unqf, paste(filesdir3, "Fields4_GLEIF.csv", sep=""))

#########################################################
# If first pass matching has been finalised
# then load newly created matrix which has the varlist here
varlist <- read.csv(paste(filesdir4, "Varlist4_GLEIF.csv", sep=""), header=TRUE)
...
write.csv(fdb, paste(filesdir5, "Output4_GLEIF.csv", sep=""))
